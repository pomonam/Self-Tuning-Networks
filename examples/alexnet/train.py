...
from examples.alexnet.model import *
...
parser = argparse.ArgumentParser(description="AlexNet STN Experiment")
parser.add_argument("--experiment_name", type=str, default="delta_stn_alexnet")
...
if info["tune_dropout"]:
    ...
    h_container.register("dropout3",
                         info["initial_dropout_value"],
                         info["scale"],
                         min_range=0., max_range=0.95,
                         discrete=False, same_perturb_mb=False)

    h_container.register("dropout4",
                         info["initial_dropout_value"],
                         info["scale"],
                         min_range=0., max_range=0.95,
                         discrete=False, same_perturb_mb=False)

    h_container.register("dropout5",
                         info["initial_dropout_value"],
                         info["scale"],
                         min_range=0., max_range=0.95,
                         discrete=False, same_perturb_mb=False)
...
model = StnAlexNet(h_container=h_container, num_hyper=num_hyper, num_classes=10)
...
if info["delta_stn"]:
    ...
    if info["optimizer"] == "adam":
        ...
        scale_optimizer = torch.optim.Adam([h_container.h_scale],
